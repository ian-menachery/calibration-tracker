...
Current phase: **v2 measured-edge, Phases 0–2 shipped** (favorite-longshot measurement) on top of the v1.1 base. New stages `backfill-created`, `tick-coverage`, `flb`; new module `analysis/recalibration.py`; new column `markets.created_at`. See ARCHITECTURE.md §14. Headline: FLB at T-24h `b≈0.88` / T-7d `b≈0.72`, concentrated in sports; v1.1 result still holds (sports near chance, politics carries signal). 100 tests; ruff clean. Phase 3 (sized rule) and Phase 4 (forward test) are designed but **deliberately not built** — do not start them without an explicit go-ahead. Note: the checked-in `.venv` was rebuilt against system Python 3.12 (the old one pointed at a dead anaconda interpreter).
