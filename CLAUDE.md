...
- **Phase 5 (Kalshi cross-venue):** `markets.venue` column; `kalshi/` module (no signing/`cryptography`); `kalshi-discover`, `kalshi-fetch-candles`, `cross-venue`. Finding: Polymarket's sports FLB does **not** replicate on Kalshi (sports@24h b≈1.0, though small n).
