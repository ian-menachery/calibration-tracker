...
Current phase: **v1.1 done** (Phase 7 + tag-based categorization). Slug-heuristic categorizer replaced with Polymarket's own `tags` taxonomy via Gamma `/events/{id}`; `market_tags` table populated for all 4,532 markets (20,267 tag rows, 744 distinct slugs); writeup + README refreshed with the new numbers. Headline holds: sports T-7d Brier 0.236, politics 0.106, "other" bucket shrank from 370 to 83 at T-7d. 84/84 tests; ruff clean. v2 backlog: Phase 6 (multi-outcome / negRisk decomposition — adds Trump 2024), lower volume floor for category diversity, hit-rate metric for apples-to-apples Polymarket comparison, more horizons (4h / 12h / 1mo), Kalshi cross-platform comparison.
